...
struct Particles
{
    ...
    /**
     * Copies data to the other struct that are needed for the first iteration
    ...
    /**
     * Updates device data just like updateSelf but does it asynchronously
     * @param [in]  sync    - queue number
     */
    void updateSelfAsync(const int sync){
      #pragma acc update self(pos_x[0:N], pos_y[0:N], pos_z[0:N], vel_x[0:N], vel_y[0:N], vel_z[0:N], weight[0:N]) async(sync)
    }
 ...
 */
void calculate_velocity(Particles& p_in, Particles& p_out, const int N, const float dt, const int sync);
