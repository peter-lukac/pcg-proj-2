...
int main(int argc, char **argv)
{
  ...
  */
  ...
  for (int s = 0; s < steps; s++)
  {
    ...
    calculate_velocity(*particles_in, *particles_out, N, dt, 1);
    centerOfMassGPU(*particles_in, N, *com, 2);


    /// In step 4 - fill in the code to store Particle snapshots.
    if (writeFreq > 0 && (s % writeFreq == 0))
    { 
      // update current particle data asynchronously
      particles_in->updateSelfAsync(3);

      // start updating com asynchronously(in queue 2 after com computation) when particles are copied
      #pragma acc wait(3)
      com->updateSelfAsync(2);

      // particle data are ready and save them
      // if the current iteration is odd(current data are in particles_out), then particles_in is particles_out
      // and we have copy data from the last iteration
      if ( s % 2 == 1 )
        // in fact it is particles_out -> particles_in
        particles_in->copyTo(*particles_out);
      h5Helper.writeParticleData(s / writeFreq);

      // when com data are ready, save them
      #pragma acc wait(2)
      h5Helper.writeCom(com->pos_x[0], com->pos_y[0], com->pos_z[0], com->weight[0], s / writeFreq);
  ...
  #pragma acc wait(2)
  com->updateSelf();
  float4 comOnGPU = {com->pos_x[0], com->pos_y[0], com->pos_z[0], com->weight[0]};
