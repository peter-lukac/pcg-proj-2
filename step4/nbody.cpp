...
void calculate_velocity(Particles& restrict p_in, Particles& restrict p_out, const int N, const float dt, const int sync){
  #pragma acc parallel loop gang present(p_in, p_out) async(sync)
  for (int i = 0; i < N; i++) {
...
void centerOfMassGPU(const Particles& restrict p,
                       const int        N,
                       Particles& restrict com,
                       const int sync)
{ 
  ...
  #pragma acc parallel loop gang present(p, com) async(sync)
  for (int i = 0; i < N; i++){
    com.pos_x[i] = p.pos_x[i];
    com.pos_y[i] = p.pos_y[i];
    com.pos_z[i] = p.pos_z[i];
    com.weight[i] = p.weight[i];
  }
  ...
  for(int i = 1; i < N_2; i <<= 1) {
    ...
    #pragma acc parallel loop gang present(com) async(sync)
    for (int j = 0; j < N;  j+=2){
      if(i + j < N) {
        const float dx = com.pos_x[j + i] - com.pos_x[j];
        const float dy = com.pos_y[j + i] - com.pos_y[j];
        const float dz = com.pos_z[j + i] - com.pos_z[j];

        float dw = ((com.weight[j + i] + com.weight[j]) > 0.0f)
                        ? ( com.weight[j + i] / (com.weight[j + i] + com.weight[j])) : 0.0f;

        com.pos_x[j] += dx * dw;
        com.pos_y[j] += dy * dw;
        com.pos_z[j] += dz * dw;
        com.weight[j] += com.weight[j + i];
      }
    }
  }
